# Condicionales, handlers y bucles
...
Para obtener más información, consulte la documentación: <https://jinja.palletsprojects.com/en/3.0.x/templates/>
...
Para ello, primero edite el inventario para agregar otro grupo y coloque `nodo2` en él. Aseegurese que esa dirección IP del `nodo2` es siempre la misma cuando lista el `nodo2`. Edite el inventario `~/ansible/inventario/hosts` para que se parezca a la siguiente lista:
...
Ejecútelo y examine la salida. El resultado esperado: la tarea se omite en nodo1, nodo3 y el host ansible (su host de control) porque no están en el grupo ftpserver del archivo de inventario.
...
Primero necesitamos el archivo que Ansible implementará, vamos a tomar el del nodo1. Recuerde reemplazar la dirección IP que se muestra en la lista siguiente con la dirección IP de su `nodo1`.

```
[ansible@ansible ansible-files]$ scp 10.243.64.42:/etc/httpd/conf/httpd.conf ~/ansible-files/files/.
ansible@11.22.33.44's password:
httpd.conf             
```
...
```bash
[ansible1@ansible ansible-files]$ curl http://22.33.44.55
curl: (7) Failed connect to 22.33.44.55:80; Connection refused
[ansible1@ansible ansible-files]$ curl http://22.33.44.55:8080
<body>
...
Para mostrar la función de bucles generaremos tres nuevos usuarios en `nodo1`. Para ello, cree el archivo `loop_users.yml` en `~/ansible-files` en el nodo de control como usuario ansible. Usaremos el módulo `user` para generar las cuentas de usuario.
...
Comparacion del playbook y la salida
<!-- {% raw %} -->
...
Compruebe que el usuario `dev_user` se creó efectivamente en `nodo1`:

```bash
[ansible@ansible ansible-files]$ ansible nodo1 -m command -a "id dev_user"
nodo1 | CHANGED | rc=0 >>

# Taller - Workflows. ( Flujos de trabajo )
...
La idea básica de un flujo de trabajo es vincular varios job template juntos. Pueden o no compartir inventario, playbooks o incluso permisos. Los enlaces entre ellos pueden ser condicionales:

  - si el job template A finaliza correctamente, se ejecuta automáticamente después el job template B.
  - En caso de error, se ejecuta el job template C.

Y los flujos de trabajo ni siquiera se limitan a Job Templates, sino que también pueden incluir actualizaciones de proyectos o inventarios.

Esto ofrece nuevas aplicaciones para Ansible AWX: pueden definirse nuevos Job Template sobre otros. Por ejemplo, el equipo de redes crea playbooks para su entorno, en su propio repositorio de Git y gestiona su propio inventario. El equipo de Operacines también utiliza sus propios repo, playbooks e inventarios.
...
Tiene dos departamentos en su organización:
  ...
  - El equipo de operaciones web que está desarrollando playbooks en su propia rama git llamada `webops`

  - El equipo de desarrolladores web que está desarrollando playbooks en su propia rama Git llamada `webdev`.

Cuando se requiere desplegar un nuevo servidor con Node.js:

**Equipo de operaciones web**:
- Debe instalar NodeJs, configurar el firewall para abrir puertos y lanzar Node.JS.

**Equipo de desarrolladores web**
- Deben desplegar las nuevas versiones desarrolladas de su aplicación Web.

---

Para hacer las cosas un poco más fáciles , todo lo necesario ya existe en un repositorio Github: Playbooks, Arhivos JSP, etc. Sólo necesitas copiarlo y pegarlo.
...
<table>
  ...
  <tr>
    <td>NAME</td>
    <td>[Academia] Webops Git Repo</td>
  </tr>
  <tr>
    <td>ORGANIZATION</td>
    <td>Default</td>
...
> **Consejo**
>
> El uso de la  "x" le permite eliminar el nodo, el signo más verde le permite agregar el siguiente nodo y los vínculos de símbolo de cadena a otro nodo.

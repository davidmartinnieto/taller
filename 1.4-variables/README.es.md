...
# Objetivos
Ansible admite variables para almacenar valores que se pueden utilizar en los Playbooks. Las variables se pueden definir en una variedad de lugares y tienen una prioridad clara. Ansible sustituye la variable por su valor cuando se ejecuta una tarea.
...
La práctica recomendada para proporcionar variables en el inventario es definirlas en archivos ubicados en dos directorios denominados `host_vars` y `group_vars`:
  ...
  - Para definir variables específicamente para un host `nodo1`, se crea el archivo `host_vars/nodo1.yml` con las definiciones de variables.
...
En el host de control ansible, cree los directorios que contendran las definiciones de variables en `~/ansible-files/inventario`:

```bash
[ansible@ansible inventario]$ mkdir host_vars group_vars
```
...
Ahora cree dos archivos que contengan definiciones de variables. Definiremos una variable denominada `stage` que apuntará a diferentes entornos, `dev` o `prod`:
  ...
  - Cree el archivo `~/ansible-files/inventario/group_vars/web.yml` con este contenido:

```yaml
---
stage: dev
```
  - Cree el archivo `~/ansible-files/inventario/host_vars/nodo2.yml` con este contenido:
...
¿De qué se trata esto?
  ...
  - Para el servidor `nodo2` esto se anula y el host se marca como un servidor de producción.
...
```bash
[ansible@ansible ansible-files]$ ansible-playbook deploy_index_html.yml
```
...
[ansible@ansible ansible-files]$ grep nodo ~/lab_inventory/hosts
nodo1 ansible_host=10.243.64.42
nodo2 ansible_host=10.243.64.43
nodo3 ansible_host=10.243.64.44


[ansible@ansible ansible-files]$ curl http://10.243.64.42
...
[ansible@ansible ansible-files]$ curl http://22.33.44.55
<body>
<h1>This is a production webserver, take care!</h1>
</body>


[ansible@ansible ansible-files]$ curl http://33.44.55.66
<body>
<h1>This is a development webserver, have fun!</h1>
</body>
```
...
```bash
[ansible@ansible ansible-files]$ ansible nodo1 -m setup
```
...
```bash
[ansible@ansible ansible-files]$ ansible nodo1 -m setup -a 'filter=ansible_eth0'
```
O qué pasa con sólo buscar facts relacionados con la memoria:

```bash
[ansible@ansible ansible-files]$ ansible nodo1 -m setup -a 'filter=ansible_*_mb'
```
...
```bash
[ansible@ansible ansible-files]$ ansible nodo1 -m setup|grep distribution
[ansible@ansible ansible-files]$ ansible nodo1 -m setup -a 'filter=ansible_distribution' -o
```
...
```bash
[ansible@ansible ansible-files]$ ansible-playbook facts.yml
...
ok: [nodo1] =>
  msg: The default IPv4 address of nodo1 is 172.16.190.143
ok: [nodo2] =>
  msg: The default IPv4 address of nodo2 is 172.16.30.170
ok: [nodo3] =>
  msg: The default IPv4 address of nodo3 is 172.16.140.196
ok: [ansible] =>
  msg: The default IPv4 address of ansible is 172.16.2.10

PLAY RECAP *********************************************************************
ansible                    : ok=2    changed=0    unreachable=0    failed=0   
nodo1                      : ok=2    changed=0    unreachable=0    failed=0   
nodo2                      : ok=2    changed=0    unreachable=0    failed=0   
nodo3                      : ok=2    changed=0    unreachable=0    failed=0   
```

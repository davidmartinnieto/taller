...
A su equipo de operaciones y a su equipo de desarrollo de aplicaciones le gusta lo que ven en Ansible AWX. Para realmente usarlo en su entorno, definieron estos requisitos:
...
<!-- {% raw %} -->
```bash
[ansible@ansible ~]$ ansible web -m command -a "curl -s http://{{ ansible_host }}"
...
Compruebe los resultados de nuevo desde el host de control del Tower. Puesto que recibimos una advertencia la última vez usando `curl` a través del módulo `command`, esta vez usaremos el módulo `uri` dedicado. Como argumentos, necesita la dirección URL real y un flag para generar el cuerpo en los resultados.
...
Ya se han hecho todos los pasos necesarios en el laboratorio. Cualquier duda puedes consultarla en el propio laboratorio.
